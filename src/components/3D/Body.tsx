...
export default function Body({
  ...
  depthBokeh: RefObject<DepthOfFieldEffect>;
}): JSX.Element {
  const [vrm, animate, load] = useSceneStore((state) => [
    state.vrm,
    state.animate,
    state.load,
  ]);
  const [playing, setPanel] = useGameStore((state) => [
    state.playing,
    state.setPanel,
  ]);
  ...
  let helperRoot: Object3D | undefined;
  useEffect(() => {
  ...
  const followCamera = useCallback(
    (camera: Camera, returnCamera: NodeJS.Timeout) => {
      ...
      if ((camera as PerspectiveCamera).fov < 150)
  ...
  useFrame(({ camera }, delta) => {
    ...
    if (resultsRef.current && resultsRef.current.rightHandLandmarks) {
      ...
      if (depthBokeh.current && depthBokeh.current.bokehScale < 20)
        depthBokeh.current.bokehScale++;
